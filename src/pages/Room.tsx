...
export default function Room() {
  ...
  const preventPointer =
    player.id !== room?.owner?.info.id ? "pointer-events-none" : "";
  ...
  const handlePickGameType = (gameType: GameType) => {
    if (room?.owner.info.id !== player.id) return;
    socketService.emit("change_game_type", {
      room_id: room?.id,
      game_type: gameType,
    });
  };
  ...
  return !isLoading && room?.id ? (
    <section className="w-full sm:w-[60%] lg:w-[40%] flex flex-col items-center justify-center gap-10">
      <Logo width={10} height={10} />
      <button
        className="bg-yellow-400 rounded-full w-15 px-3 py-3 text-black-300 top-5 left-5 absolute"
        onClick={onLeaveRoom}
      >
        Leave Room
      </button>
        ...
        <article className="bg-black-400 py-3 rounded-lg flex w-full mb-5">
          ...
          >
            <p
              className={
                pickGameType === GameType.SUMOKU
                  ? `text-black-400 font-bold text-lg`
      ...
      <article className="flex flex-col gap-3 w-[90%]">
        {room.owner.info.id === player.id && (
          <div className="w-full bg-yellow-500 rounded-2xl pb-2">
            <Btn
              classCSS={`bg-yellow-400 rounded-2xl w-full py-2 ${preventPointer}`}
              onClick={() => onStartGame()}
            >
              START GAME
            </Btn>
        ...
        <div className="pb-2 cursor-auto flex flex-row gap-2 w-full">
          ...
          {room?.competitor ? (
            <div className="bg-blue-400 rounded-2xl w-full flex justify-center text-black-400 text-[15px] p-3 py-4 basis-1/2 items-center gap-x-1">
              ...
              {room.owner.info.id === player.id && (
                <button
                  className="bg-red-400 justify-center rounded-full min-w-[40px] w-10 min-h-[40px] h-10 px-3 flex items-center relative group"
                  onClick={() =>
                    room?.competitor?.info?.id &&
                    onKick(room?.competitor?.info?.id)
                  }
                >
                  <p>X</p>
            ...
            <div className="bg-blue-400 rounded-2xl w-full py-2 flex justify-around text-black-400 text-[15px] p-3 basis-1/2">
              ...
              <button
                className="bg-red-400 rounded-full w-15 px-3 py-3"
                onClick={onAddBot}
              >
                Add Bot
              </button>
