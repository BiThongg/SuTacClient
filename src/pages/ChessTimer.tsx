...
const symbol: Record<Cell, JSX.Element> = {
  [Cell.X]: <img src={IconX} className="w-6" />,
  [Cell.O]: <img src={IconO} className="w-6" />,
  [Cell.EMPTY]: <></>,
};
...
const ChessTimer: React.FC<IChessTimerProps> = ({ game, initialTime }) => {
  ...
  useEffect(() => {
    setCurrentState({
      title: mySelf?.symbol == game.turn ? "Your Turn" : "Opponent Turn",
      currentSymbol: game?.turn.valueOf(),
    });
    resetTimer();
  ...
  return (
    <div className="flex flex-col items-center p-6 bg-gray-800 text-white rounded-lg">
      ...
      <div className="text-4xl font-bold mb-4 mt-2">
        {game.turn.valueOf() === mySelf?.symbol.valueOf() ? (
          formatTime(timeLeft)
        ) : (
          <span className="flex flex-row gap-3">
            <p>Waiting </p> <RingSpin />{" "}
          </span>
        )}
      </div>
      ...
      <div className="flex flex-row gap-3">
        {game.turn.valueOf() === mySelf?.symbol.valueOf() ? (
          <div className="px-4 py-2 bg-green-500 text-black rounded-md">
            {currentState?.title}
          </div>
        ) : (
          <div className="px-4 py-2 bg-red-500 text-black rounded-md">
            {currentState?.title}
          </div>
        )}

        <div className="px-3 py-2 bg-black-300 text-white rounded-md flex">
          <span className="flex flex-row gap-2">
            <p>Your Symbol: </p> {symbol[mySelf?.symbol ?? Cell.X]}{" "}
          </span>
        </div>
      </div>
    </div>
