...
export default class RoomClass {
    ...
    isReady(): boolean {
        return (
            this.competitor?.status === Status.READY && this.owner.status === Status.READY
        );
    }

    changeStatus(userId: string): void {
        const participant = this.owner.info.id === userId ? this.owner : this.competitor;
        if (participant) {
            participant.status = participant.status === Status.WAITING ? Status.READY : Status.WAITING;
        }
    }
